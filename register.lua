...
local function get_color_buttons(coleft, lotop)
	...
	for y = 0, 3 do
		for x = 0, 3 do
			local file_name = "custom_colorbutton"..(y * 4 + x)..".png"
	...
	if data.infotext then
		...
		if page == "main" then
			formspec = formspec.."image_button["..(data.hileft+2.1)..",0.1;0.8,0.8;"
					.."custom_pencil_icon.png;edit_infotext;]"
					.."label["..(data.hileft+3)..",0;"..formspec_infotext.."]"
		elseif page == "edit_infotext" then
			formspec = formspec.."image_button["..(data.hileft+2.1)..",0.1;0.8,0.8;"
					.."custom_checkmark_icon.png;save_infotext;]"
...
local function get_receive_fields(name, data)
	...
	return function(pos, formname, fields, sender)
		...
		if data.color then
			-- This sets the node
			local nn = "custom:"..lname..(data.locked and "_protected" or "").."_chest"
...
function custom.chests:definition(name, data)
	local lname = name:lower()
	name = S(name)
	local d = {}
	...
	local front = {"custom_"..lname.."_chest_front.png"}
	data.base_formspec = "size["..data.ovwidth..","..data.ovheight.."]"..
			...
			"background[-0.19,-0.25;"..(data.ovwidth+0.4)..","..(data.ovheight+0.75)..";custom_form_bg.png]"..
			"background["..data.hileft..",1;"..data.width..","..data.height..";custom_"..lname.."_chest_inventory.png]"..
			"background["..data.loleft..","..data.lotop..";8,4;custom_main_inventory.png]"
	if data.sort then
	...
	if data.locked then
		locked_after_place = function(pos, placer)
			...
			meta:set_string("infotext",
					S("%s Protected Chest (owned by %s)")
					:format(name, meta:get_string("owner")))
			pipeworks.after_place(pos)
		end
		table.insert(front, "custom_"..lname.."_chest_lock_overlay.png")
	else
	...
	local def = {
		description = desc,
		tiles = {"custom_"..lname.."_chest_top.png", "custom_"..lname.."_chest_top.png",
			"custom_"..lname.."_chest_side.png", "custom_"..lname.."_chest_side.png",
			"custom_"..lname.."_chest_side.png", table.concat(front, "^")},
		paramtype2 = "facedir",
		groups = self.groups,
		tube = self.tube,
...
function custom.chests:register(name, data)
	local def = custom.chests:definition(name, data)

	local nn = "custom:"..name:lower()..(data.locked and "_protected" or "").."_chest"
	...
	if data.color then
		...
		for i = 1, 15 do
			...
			colordef.tiles = { def.tiles[1], def.tiles[2], def.tiles[3], def.tiles[4], def.tiles[5], mk_front("custom_chest_overlay"..postfix..".png") }
